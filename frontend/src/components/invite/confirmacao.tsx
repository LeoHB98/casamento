...
export function Confirmacao(props: ConfirmacaoProps) {
  ...
  function handleSetSendCode(event: FormEvent) {
    ...
    if (localCode && localCode.trim().length > 0) {
      props.setCode(localCode.toUpperCase());
  ...
  return (
    <div className={styles.content}>
      <CloseButton setBoolean={props.setState} setNullString={props.setCode} />

      <p>Código disponibilizado pelo(a) noivo(a)</p>

      <form onSubmit={handleSetSendCode} className={styles.inputSpace}>
        <input
          name="comment"
        ...
        <div className={styles.sendButton}>
          <button type="submit" title="Enviar codigo">
            Enviar
          </button>
        </div>
      </form>
