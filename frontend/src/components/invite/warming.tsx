...
export default function WarmingCode(props: WarmingProps) {
  ...
  const handleToNextPart = useCallback(() => {
  ...
  return (
    <div className={styles.container}>
      {props.warning === "ok" ? (
        ...
        <>
          <p>Voce já confirmou a presença.</p>
          <p>Deseja editar?</p>

          <div className={styles.options}>
            <button onClick={close}>Não</button>

            <button onClick={handleToNextPart}>Sim</button>
          </div>
        </>
        ...
        <Modal currentState>
          <CloseButton setBoolean={props.setToValueBool1} />
